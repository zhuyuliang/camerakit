...
public class UsbMonitor implements IMonitor {
    ...
    public boolean isTargetDevice(UsbDevice usbDevice) {
        if (!isUsbCamera(usbDevice)
            ...
            LogUtil.i("No target camera device" + usbDevice.getProductId() + " " + mConfig.getProductId());
            LogUtil.i("No target camera device1" + usbDevice.getVendorId() + " " + mConfig.getVendorId());
            return false;
        }
